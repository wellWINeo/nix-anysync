...
pkgs.testers.nixosTest {
  ...
  testScript = ''
    ...
    server.succeed("for i in $(seq 1 30); do mongosh --host 127.0.0.1:27017 --quiet --eval 'db.hello()' 2>/dev/null | grep -q 'isWritablePrimary.*true' && exit 0; sleep 1; done; exit 1")
    server.succeed("mongosh --host 127.0.0.1:27017 --quiet --eval 'db.hello()' | grep -q 'setName.*rs0'")
